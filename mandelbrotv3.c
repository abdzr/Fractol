...
/*   Updated: 2019/06/01 20:48:45 by azarzor          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     mandelv3calc(t_env *env, t_threads thread)
{
    ...
    while (thread.x * thread.x + thread.y * thread.y <= 4 && iter < env->max)
			{
				thread.xnew = thread.x * thread.x * thread.x - 3 * thread.x * thread.y * thread.y + thread.cre;
				thread.y = 3 * thread.x * thread.x * thread.y - thread.y * thread.y * thread.y + thread.cim;
